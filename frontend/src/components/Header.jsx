    ...
    if (storedEmail && storedLoginStatus === "true") {
      ...
      axios.get(`http://localhost:5000/student/${storedEmail}`)
        .then((response) => {
          ...
          setUserRegistered(student.registered === "yes"); // Set registered state
        })
        .catch((error) => {
          console.error("Error fetching student data:", error);
          toast.error("Failed to fetch student data.", { position: "bottom-center", autoClose: 3000 });
        });
    }
  }, [isLogin]);
  ...
  return (
    <div className='w-[90%] mt-[2rem] gap-[1rem] md:gap-0 mx-auto flex flex-col md:flex-row justify-between items-center  mb-[2rem]'>
      ...
      <div className="flex flex-row mobile:flex-col mobile:gap-[0.8rem] md:flex-row items-center gap-[4rem] md:gap-[4rem] text-[#72265F]">
        ...
        to="placement_team"
        smooth={true}
        duration={800} // Adjust duration for smoother animation
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-48 top-full left-0 bg-white shadow-lg border rounded-md z-50">
              ...
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem disabled={!userRegistered} onClick={() => navigate("/edit-profile")}>
                  <User className="mr-2 h-4 w-4" />
                  <span>Edit Profile</span>
                </DropdownMenuItem>


                <DropdownMenuItem disabled={!userRegistered} onClick={() => navigate("/companylistings")}>
                  <User className="mr-2 h-4 w-4" />
